...
for np in $(seq 2 2 256); do
  for i in {1..10}; do
    if [ $i -eq 1 ]; then
      mpirun -np $np ./parallel_quicksort_strong ${DATA_SIZE} 1 timings_strong_scaling.csv
    else
      mpirun -np $np ./parallel_quicksort_strong ${DATA_SIZE} 0 timings_strong_scaling.csv
    fi
  done
done
